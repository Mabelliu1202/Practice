...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{1.png} % Uncomment and use for actual image
    \caption{\textbf{Dartboard and its respective scoring zones}}
\end{minipage}
\end{figure}

The dartboard encompasses 82 distinct scoring zones, each with a corresponding point value. Any area outside the board is assigned a score of zero. The symbols and scores for each zone are detailed in Table 1, where k represents the score value of the outermost ring, k \in \{1, 2, \dots, 20\}.\end{aligned}
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{2.png} % Uncomment and use for actual image
...
\begin{aligned}
T &= \{ 0, 1, \cdots \}\end{aligned}is the decision time point set, and the time point t at which a darts player chooses a target for throwing is the moment of decision. Considering that some players’ skills may be at very low levels and they are unable to complete the game, T in this context is an infinite set.


S &= \{501,500,499,\cdots\,2,0\}\
\end{aligned}is the state space and represents the player's current remaining score. It is important to note that there is no state of "1" because of the "double out rule".


A &=\{ DB,SB,〖SC〗_k  ,〖SD〗_k  ,T_k  ,D_k\}\ 
\end{aligned} is the action set, where k \in \{ 1, 2, \cdots, 20 \}\end{aligned}. The action set consists of the 82 target positions on the dartboard, which correspond to the target set defined in 3.1 section.


p(s_(t+1) s_t,a)
\end{aligned}is the transition probability. It refers to the probability of the system transitioning to a states
s_{t+1} \in S\end{aligned}at the next decision moment after the player, in states_t \in S\end{aligned}, takes an action a \in A\end{aligned}(i.e., aiming at a specific position and throwing), and \sum_{s_{t+1} \in S} p(s_{t+1} \mid s_t, a) = 1\end{aligned}.For example, if a player is in state s_t = 300\end{aligned} and takes an action a = T_{20}\end{aligned}, meaning they aim at a = T_{20}\end{aligned}and throw, they might hit a = T_{20}\end{aligned}, ut they could also hit areas around, ut they could also hit areas around,such as T_1, T_5, SC_5\end{aligned},etc.Thus, the next state s_{t+1}\end{aligned}could be 240, 297, 285, 295, and so on.


r(s_t,a)\end{aligned}is the immediate reward. According to the rules of the 501 games, when a player is in state s_t\end{aligned} and makes a throw a , the number of throws increases by one. Define
r(s_t, a) := 1\end{aligned},which acts as a counter in the subsequent optimization process.

When constructing the Dart-MDP model for a darts player, the set of decision time points T, state space S, action set A, and reward function r(s_t, a)\end{aligned} are all fixed. The only unknown is the transition probability p(s_{t+1} \mid s_t, a)\end{aligned},which is determined by the player's dart skill. The following discusses how to calculate the transition probability.

\subsection{Solving the Transition Probability}

The transition probability p (s_{t+1} \mid s_t, a)\end{aligned} represents the probability that, when the player is in state s_t \in S\end{aligned}and takes an action a\in A \end{aligned}(i.e., aims at a target a and throws), the system transitions to state s_{t+1} \in S\end{aligned} at the next decision time point. This can be expressed as the probability of the event that the player, aiming at a \in A\end{aligned} and throwing, achieves a score of(s_t - s_{t+1})\end{aligned}.
...
\end{aligned}. The score at a position z on the dartboard is denoted by \text{Score}(z)\end{aligned}.

Using the method proposed by Ryan J.T. et al. [6], a player's skill level is characterized by the standard deviations (\sigma_X, \sigma_Y)\end{aligned} of two independent bivariate normal distributions in the X and Y directions. The probability of hitting a region h = \text{hit} \end{aligned}after aiming at the \text{target} = a\end{aligned} can be calculated using the integral formula of a bivariate normal distribution as follows:\end{aligned}
...
(\mu_{aX}, \mu_{aY})\end{aligned} are the X- and Y-coordinates of the centroid of the target region.

(\sigma_X, \sigma_Y)\end{aligned} are the standard deviations reflecting the player's throwing accuracy in the X-\end{aligned} and Y-\end{aligned} directions. 

Combining equations (1), (2), and (3), we can derive the formula for calculating the formula for p(s_{t+1} \mid s_t, a)\end{aligned} as blow:\end{aligned}
\end{equation}
...
The above derivation highlights that the standard deviations(\sigma_X, \sigma_Y)\end{aligned}of the bivariate normal distribution are critical determinants of the transition probability. The following section explores methods for estimating a player's skill level.

\subsection{Estimating Dart Throwing Accuracy}

Ryan J.T. and his colleagues utilized the Expectation-Maximization (EM) algorithm to estimate the standard deviations(\sigma_X, \sigma_Y)\end{aligned}for different dart players [6].

Smaller standard deviations indicate a higher skill level for the players. However, since elderly individuals often have limited physical stamina, it can be challenging to collect sufficient trial throw data to apply the EM algorithm. Therefore, in community practice, this study employs a nearest-neighbor estimation method (referred to as the "nearest-neighbor estimation method") to estimate the dart throwing accuracy of dart players.
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{3.png} % Uncomment and use for actual image
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{4.png} % Uncomment and use for actual image
...
The set of all optimal decisions across the entire state space S constitutes the player's optimal strategy\{ a_{s_t}^* \mid s_t \in S \}\end{aligned}.
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{5.png} % Uncomment and use for actual image
    \caption{\textbf{State Transition Diagram for a Player Aiming at Target a in State $s_t=i$}}
\end{minipage}
\end{figure}
...
By reformulating Equation (6), the mathematical expectation of the minimum number of throws required for a player to complete the game when in state s_t\end{aligned} and taking action a\end{aligned} can be expressed as:
...
Following the principles of dynamic programming, we first calculate E_{\min}[2]\end{aligned}. Subsequently, by iterating backwards, we compute the optimal decision for all states, thereby deriving the optimal strategy for the player.

To illustrate this process, consider a player with(\sigma_X = 1.8, \sigma_Y = 2.2)\end{aligned}. Figure 4 depicts the state transition diagram when this player is in state s_t = 2\end{aligned} and takes different actions a to transition to the next state s_{t+1} = 0\end{aligned}.
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{6.png} % Uncomment and use for actual image
    \caption{\textbf{Transition diagrams for a player in state $
s_t = 2$ aiming at different targets}}
\end{minipage}
\end{figure}

From the state transition diagram above, the expected number of throws required to transition from s_t = 2\end{aligned} to s_{t+1} =0:\end{aligned}

when aiming at D1,E(2, D1) = 3.536\end{aligned},

when aiming at SD1,E(2, \text{SD1}) = 10.235\end{aligned}.

After calculating the expected number of throws for all 82 targets, we find that is the optimal decision in state s_t = 2\end{aligned} for the player, and E_{\min}(2) = E(2, \text{D1}) = 3.536\end{aligned}
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{7.png} % Uncomment and use for actual image
    \caption{\textbf{Transition diagrams for a player in state $ s_t=3$ aiming at two targets}}
\end{minipage}
\end{figure}

From the diagram above, it can be determined that the player's optimal decision at s_t = 3\end{aligned} is \text{SD1}\end{aligned},with E_{\min}[3] = 5.16.\end{aligned}
...
\text{\textbf{New Rule 2:}}Fo New Rule 2, the player simply needs to greedily select the location with the highest expected score at each throw. The optimal decision for each state is:
...
\textbf{Training Model Assumption}:Within a given timeframe, after a player practices aiming at a specific target location$a \in A$\end{aligned}on the dartboard a certain number of times,the standard deviations$(\sigma_X, \sigma_Y)$\end{aligned}for throws towards this location will decrease by a certain percentage, while those for other locations remain unchanged.\skip 
...
Based on the previous estimation results, Grandma Wang's overall skill level is characterized by standard deviations of 6.0 and 8.8 in the X and Y directions, respectively. Let us assume that after a period of intensive training, Grandma Wang's standard deviations when aiming at a specific location $a \in A$  are reduced by \( 20\% \)\end{aligned},resulting in $(\sigma_X = 4.8, \sigma_Y = 7.0)$\end{aligned},while accuracy at other locations remains unchanged. Applying the methodology for New Rule 1 and conducting trials across all targets on the dartboard, we obtain the following results:
\newpage

\begin{table}[h!]
\centering
\begin{minipage}{0.6\textwidth} % 表格佔 60% 寬度
...
\end{center}
\begin{document}
\textbf{\item 1. Calculating the aiming points (Centroids) for all target areas on a dartboard.}

 \text The coordinates(\mu a_X, \mu a_Y)\end{aligned} of the aiming point (centroid) for each target area (annular sector) can be calculated using the centroid formula for an annular sector. The centroid lies on the bisector of the sector angle, and its distance from the center of the dartboard is as follows：
...
\end{minipage}
\caption{Calculating the centroid of an annular sector in the dartboard}
\end{figure}
  
\textbf{\item 2. Example of transition probability solving.}

\text A player's skill level is characterized by the standard deviations
(\sigma_X, \sigma_Y)\end{algined} of two independent bivariate normal distributions in the X and Y directions. When a player aims at {\text{target}} = a\end{aligned} and hits a specific area \text{hit} = h\end{aligned},the probability of this event can be expressed using the integral formula of the bivariate normal distribution, where (\mu a_X, \mu a_Y)\end{aligned} represent the coordinates of the centroid of the \text{target} = a\end{aligned}.
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture }\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{10.png} % Uncomment and use for actual image
    \caption{\textbf{An example of solving the transition probability of the MDP}}
\end{minipage}
\end{figure}

\textbf{\item 3. A bug report concerning an anomaly in the calculation of double integrals was submitted to the maintainer of the relevant R package, and a reply from the package author was received. }
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture }\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{11.png} % Uncomment and use for actual image
    \caption{\textbf{Bug Report}}
\end{minipage}
\end{figure}

\textbf{    
\item 4.Using SQLite Studio to store and manage throwing data for robots, professional Players, and elderly Players.}
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture }\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{12.png} % Uncomment and use for actual image
    \caption{Using SQLite Studio to store and manage the experimental data}
\end{minipage}
\end{figure}
  
\textbf{\item 5. Comparison of optimal strategies for completing a 501 dart game: world champion Michael vs. Grandma Wang from Local Community.}
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture 1}\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{13.png} % Uncomment and use for actual image
    \caption{\textbf{Comparison of optimal strategies of the two players}}
\end{minipage}
\hspace{0.5cm} % Space between the images
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture 2}\rule[-1.5cm]{0pt}{3cm}} % Second image
    % \includegraphics[width=\linewidth]{14.png} % Uncomment and use for actual image
    \caption{\textbf{Comparison of optimal strategies of the two players}}
\end{minipage}
\end{figure}

\textbf{\item 6. The Darts society from Beijing No. 101 High School hosted dart events for the elderly in local communities.}
...
\begin{minipage}{0.45\textwidth}
    \centering
    \fbox{\makebox[5cm][c]{Picture }\rule[-1.5cm]{0pt}{3cm}} % First image
    % \includegraphics[width=\linewidth]{15.png} % Uncomment and use for actual image
